...
void close_buy_order(Transaction *transaction)
{
    ...
    log_message(LOG_INFO, "Buy order closed: Order ID: %d\n", transaction->buying_order->id);
}

void close_sell_order(Transaction *transaction)
{
    transaction->selling_order->status = CLOSE;
    log_message(LOG_INFO, "Sell order closed: Order ID: %d\n", transaction->selling_order->id);
}

void add_buy_order_pending_shares(Transaction *transaction, int shares)
{
    transaction->buying_order->pending_shares += shares;
    log_message(LOG_INFO, "Added %d shares to pending shares for buying order ID: %d\n", shares, transaction->buying_order->id);
}

void add_sell_order_pending_shares(Transaction *transaction, int shares)
{
    transaction->selling_order->pending_shares += shares;
    log_message(LOG_INFO, "Added %d shares to pending shares for selling order ID: %d\n", shares, transaction->selling_order->id);
}
